...
pattern = r'val displayMonth = FormatUtils.formatDisplayMonth\(currentMonth\).*?CustomFilterChip\("expense", "Chi phí \(\$\{expenseEntries.size\}\)", selectedFilter, primaryColor\) \{ selectedFilter = "expense" \}\n        \}'

content = re.sub(pattern, new_header, content, flags=re.MULTILINE | re.DOTALL)

with open("app/src/main/java/com/example/ui/screens/HistoryScreen.kt", "w") as f:
    f.write(content)

print("Updated HistoryScreen")
